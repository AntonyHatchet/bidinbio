...
async function updateStateForUser(igAccountId: string, longLiveToken: string) {
  ...
  for (const media of mediasIds) {
    await handleMentionsHook({ time: new Date(), userId: igAccountId, mediaId: media.id });
  }
  const auctions = await Auction.find({ status: "active" });
  for (const auction of auctions) {
    const comments = await loadComments(auction.mediaId, longLiveToken);
    const unansweredMessages = comments.filter(comment => auction.bids.every(bid => bid.commentId !== comment.id));
